...
def fetch_data_from_index():
    """Récupère les données entre deux indices donnés."""
    try:
        base_index = int(index_entry.get())  # Récupère l'index de début
        end_index = int(end_index_entry.get())  # Récupère l'index de fin
    except ValueError:
        print("Veuillez entrer des indices valides.")
        return

    # Vérifier que l'index de début est inférieur à l'index de fin
    if base_index >= end_index:
        print("L'index de début doit être inférieur à l'index de fin.")
        return
    ...
    for compteur in compteurs:
        ...
        # Calcul de l'index cible en tenant compte de l'offset
        start_index = base_index + compteur["offset"]
        final_end_index = end_index + compteur["offset"]
        url = f"http://{compteur['ip']}/data/?from={start_index}&to={final_end_index}"
        try:
            ...
            if "text/csv" in response.headers.get("Content-Type", ""):
                ...
                for row in csv_reader:
                    ...
                    if compteur["nom"] == "Compteur Grid":
                        compteur["energy_export"].append(int(row[6]))
                        compteur["index"].append(row[1])
                    rows.append(row)
...
tk.Label(end_frame, text="Entrer un index de fin pour compteur Grid : ").pack(side=tk.LEFT)
end_index_entry = tk.Entry(end_frame)
